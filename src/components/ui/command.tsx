...
function CommandInput({ className, ...props }: ComponentProps<typeof CommandPrimitive.Input>) {
  return (
    <div
      cmdk-input-wrapper=""
      className="order-3 mx-auto mb-2.5 flex w-[calc(100%-20px)] flex-1 items-center rounded-md border
        border-neutral-300 bg-neutral-900 px-3 md:order-1 md:mb-0 md:w-full md:rounded-none md:border-x-0
        md:border-b md:border-t-0 md:border-neutral-700"
    >
...
function CommandList({ className, ...props }: ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      className={cn(
        `max-h-[calc(80dvh-66px)] overflow-y-auto overflow-x-hidden scrollbar-none md:order-2
        md:max-h-[300px] md:scrollbar-thin`,
        className
      )}
      {...props}
    />
